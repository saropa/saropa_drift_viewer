...
| **[saropa_drift_advisor](https://pub.dev/packages/saropa_drift_advisor)**            | **Debug-only SQLite/Drift inspector.** Exposes your database as JSON and a minimal web UI: table list, pagination, schema, CSV export, read-only SQL runner, snapshot/diff, and optional auth. Use from any Flutter/Dart app with Drift or raw SQLite. |
...
- **[Saropa Drift Advisor](https://marketplace.visualstudio.com/items?itemName=Saropa.drift-viewer)**
  - _SQLite/Drift at a glance:_ Run a task to open the Saropa Drift Advisor in your browser and inspect tables, run read-only SQL, export schema or data, and compare snapshots—without leaving the editor.
