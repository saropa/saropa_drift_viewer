...
| Service | Usage |
|---------|-------|
| SchemaIntelligence | Table metadata for Table Stats and Row Count widgets |
| QueryIntelligence | Query pattern data for performance widgets |

### Consumes From

| Feature | Data/Action |
|---------|-------------|
| Health Score (30) | Health Score widget data |
| Data Invariant Checker (27) | Invariant Status widget |
| Query Replay DVR (26) | DVR Status widget |
| Real-time Mutation Stream (22) | Mutation count/rate widget |
| Query Perf Regression Detector (63) | Performance trend widget |
| Multi-Server Federation (35) | Cross-server status widget |

### Produces For

| Feature | Data/Action |
|---------|-------------|
| Standalone | Personalized debug command center |

### Cross-Feature Actions

| From | Action | To |
|------|--------|-----|
| Table Stats Widget | "View Data" | Table data viewer |
| Table Stats Widget | "Add Invariant" | Invariant Manager |
| Query Result Widget | "Edit SQL" | SQL Notebook |
| Query Result Widget | "Build Visually" | Visual Query Builder |
| Health Score Widget | "View Details" | Health Score panel |
| Invariant Widget | "View Violations" | Invariant Manager |

### Widget Registry Enhancement

Widgets connect to integration hubs:

| Widget Type | Data Source | Integration |
|-------------|-------------|-------------|
| Table Stats | SchemaIntelligence | Cached, fast refresh |
| Query Performance | QueryIntelligence | Aggregated patterns |
| Health Score | Health Score Engine | Shared with command center |
| Invariants | InvariantManager | Real-time status |
| DVR Status | QueryRecorder | Recording state |
| Mutations | MutationClient | Live event count |

### Dashboard as Command Center

The Dashboard becomes the **central hub** from the integration plan, providing one-click access to all features:

```
┌─────────────────────────────────────────────────────────┐
│ DASHBOARD — COMMAND CENTER                              │
├───────────────┬─────────────────┬──────────────────────┤
│ ❤ Health: B+ │ 🛡 5/6 Invariants│ ⏺ DVR: 147 queries │
│ [Details]     │ [View]          │ [Open Timeline]     │
├───────────────┴─────────────────┴──────────────────────┤
│ [+ Add Widget]   Drag to rearrange                     │
└────────────────────────────────────────────────────────┘
```

### Health Score Widget Actions

Each Health Score metric in the dashboard widget is actionable:

| Metric | Click Action |
|--------|--------------|
| Index Coverage | → Index Suggestions panel |
| FK Integrity | → Anomaly viewer (orphans) |
| Data Quality | → Invariant Manager |
| Query Performance | → Cost Analyzer with slowest query |
